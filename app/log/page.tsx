...
type Block  = { id: number; movementId: string; movementLabel: string; hasWeight: boolean; sets: SetRow[] }
const newBlock = (id: number): Block => ({ id, movementId: '', movementLabel: '', hasWeight: true, sets: [{ reps: '', weight: '' }] })
...
const PAIN_PARTS     = [
  'Épaule G','Épaule D','Poignet G','Poignet D','Coude G','Coude D',
  'Avant-bras G','Avant-bras D','Genou G','Genou D','Hanche G','Hanche D',
  'Bas du dos','Lombaires','Cheville G','Cheville D','Cou','Quad G','Quad D','Ischio G','Ischio D',
]
const SEVERITY = [
...
// ── Mapping sports profil → types de séance ───────────────
...
function validate(step: number, data: { date: string; typeId: string }): string | null {
  ...
  if (step === 1 && !data.typeId) return 'Sélectionne un type de séance pour continuer.'
  return null
}
...
export default function LogPage() {
  ...
  const [hasWod, setHasWod]             = useState(false)
  ...
  // Charge les types + filtre par sports du profil
  useEffect(() => {
  ...
  const updBlock = (id: number, field: Partial<Block>) =>
    setBlocks(bs => bs.map(b => b.id === id ? { ...b, ...field } : b))
  const updSet = (blockId: number, si: number, field: Partial<SetRow>) =>
    setBlocks(bs => bs.map(b => b.id === blockId
      ? { ...b, sets: b.sets.map((s, i) => i === si ? { ...s, ...field } : s) } : b))

  // Photo → pré-remplit warm-up + WOD
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.strength_notes) { setSkillNotes(data.strength_notes); filled.push('skill/force')  }
      if (data.format)         { setHasWod(true); setWodFormat(data.format) }
      if (data.description)    { setWodDesc(data.description);       filled.push('WOD')          }

      toast.success(filled.length > 0
        ? `Tableau analysé ✓ — ${filled.join(', ')} pré-remplis`
        : 'Photo analysée ✓'
      )
    } catch (err) {
  ...
  const reset = () => {
    ...
    setWarmupNotes(''); setSkillNotes(''); setPhotoPreview(null); setBlocks([newBlock(Date.now())])
    setRunType(''); setRunDistance(''); setRunElevation(''); setRunSurface('')
    setHasWod(false); setWodFormat(''); setWodDesc(''); setWodResult(''); setWodRx(true)
    setRpe(7); setFeeling(3); setPainEntries([]); setNotes('')
  ...
  const handleSubmit = async () => {
    ...
    try {
      const allNotes = [
        ...
        skillNotes  ? `Skill/Force: ${skillNotes}`   : '',
        notes,
      ].filter(Boolean).join('\n') || undefined
      ...
      const id = await saveSession({
        ...
        rpe, feeling_post: feeling,
        notes: allNotes,
        blocks: isRun ? [] : blocks.filter(b => b.movementId).map(b => ({
        ...
        wod: !isRun && hasWod && wodFormat ? {
          format_label:  wodFormat, description: wodDesc || undefined,
          result_detail: wodResult || undefined, is_rx: wodRx,
        } : undefined,
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="max-w-sm w-full bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-100">
          ...
          <p className="text-gray-400 text-sm mb-6">
            {date}{duration ? ` · ${duration} min` : ''}
            {isRun && runDistance ? ` · ${runDistance} km` : ''}
          </p>
          <div className="flex gap-3">
            <Link href="/" className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 text-sm font-semibold text-center hover:bg-gray-50 transition">
              ← Tableau de bord
            </Link>
            <button onClick={reset} className="flex-1 text-white rounded-xl py-3 font-semibold transition text-sm"
              ...
              <div className="grid grid-cols-3 gap-2">
                {sessionTypes.map(t => (
                  <button key={t.id} onClick={() => { setTypeId(t.id); setError(null) }}
                    className={`p-3 rounded-xl border text-xs font-medium transition flex flex-col items-center gap-1 ${
                      typeId === t.id ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-gray-300 bg-white text-gray-600 hover:border-gray-400'
                    }`}>
          ...
          <div className="space-y-6">
                ...
                {RUN_TYPES.map(t => (
                  <button key={t} onClick={() => setRunType(t)}
                    className={`px-4 py-2 rounded-full text-sm font-medium border transition ${
                      runType === t ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-gray-300 bg-white text-gray-600'
                    }`}>{t}</button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" step="0.1" placeholder="ex: 10" value={runDistance}
              ...
              <div>
                ...
                <input type="number" placeholder="ex: 250" value={runElevation}
            ...
            {paceStr && (
              <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-xs font-bold text-green-700 uppercase tracking-wide">Allure calculée</p>
                  <p className="text-xl font-bold text-green-600">{paceStr}</p>
                </div>
              </div>
            )}
            <div>
                ...
                {RUN_SURFACES.map(s => (
                  <button key={s} onClick={() => setRunSurface(s)}
                    className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition ${
                      runSurface === s ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-gray-300 bg-white text-gray-600'
                    }`}>{s}</button>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            {photoPreview ? (
              <div className="relative rounded-xl overflow-hidden border border-gray-200">
                ...
                <img src={photoPreview} alt="Tableau" className="w-full max-h-40 object-cover" />
                {analyzingPhoto && (
                ...
                {!analyzingPhoto && (
                  <button onClick={() => { setPhotoPreview(null); if (photoInputRef.current) photoInputRef.current.value = '' }}
                    className="absolute top-2 right-2 w-7 h-7 bg-white rounded-full shadow flex items-center justify-center text-gray-500 hover:text-red-400 transition text-sm">
                    ×
                  </button>
                )}
              </div>
              ...
              <button onClick={() => photoInputRef.current?.click()} disabled={analyzingPhoto}
                className="w-full py-3.5 rounded-xl border-2 border-dashed text-sm font-semibold flex items-center justify-center gap-2 transition border-gray-300 text-gray-500 hover:border-orange-300 hover:text-orange-500 hover:bg-orange-50">
                <span>📷</span> Analyser le tableau — pré-remplit warm-up + WOD
              </button>
            )}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4">
              ...
              <textarea rows={2} value={warmupNotes} onChange={e => setWarmupNotes(e.target.value)}
                placeholder="Ex: 2 rounds — 10 air squat, 10 push up, 200m run..."
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4">
              ...
              <textarea rows={2} value={skillNotes} onChange={e => setSkillNotes(e.target.value)}
                placeholder="Ex: Snatch technique 15 min — focus position basse..."
                className={inputCls + " resize-none"} />
            </div>

            {/* Blocs de force */}
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wide pt-1">Force — optionnel</p>
            {blocks.map((block, bi) => (
              <div key={block.id} className="bg-white rounded-2xl border border-gray-200 p-4">
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-6 h-6 rounded-md bg-orange-100 text-orange-600 flex items-center justify-center text-xs font-bold flex-shrink-0">{bi + 1}</span>
                  <span className="text-sm font-bold text-gray-700 flex-1">{block.movementLabel || `Bloc ${bi + 1}`}</span>
                  {blocks.length > 1 && (
                    <button onClick={() => setBlocks(bs => bs.filter(b => b.id !== block.id))}
                      className="text-gray-300 hover:text-red-400 text-xl leading-none">×</button>
                  )}
                </div>
                <div className="mb-3">
                  <MovementSearch value={block.movementLabel}
                    onChange={m => updBlock(block.id, {
                      movementId: m.id, movementLabel: m.name,
                      hasWeight: !NO_WEIGHT_CATS.includes(m.category),
                    })} />
                </div>
                <div className="flex items-center gap-2 mb-3">
                  <button onClick={() => updBlock(block.id, { hasWeight: !block.hasWeight })}
                    className={`w-9 h-5 rounded-full relative transition-colors flex-shrink-0 ${block.hasWeight ? 'bg-orange-500' : 'bg-gray-200'}`}>
                    <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${block.hasWeight ? 'left-4' : 'left-0.5'}`} />
                  </button>
                  <span className="text-xs text-gray-500">{block.hasWeight ? 'Avec poids (kg)' : 'Reps uniquement'}</span>
                </div>
                <div className={`grid gap-2 text-xs text-gray-400 uppercase font-semibold px-0.5 mb-1.5 ${block.hasWeight ? 'grid-cols-[20px_1fr_1fr_24px]' : 'grid-cols-[20px_1fr_24px]'}`}>
                  <span/><span>Reps</span>{block.hasWeight && <span>Poids kg</span>}<span/>
                </div>
                {block.sets.map((set, si) => (
                  <div key={si} className={`grid gap-2 mb-1.5 items-center ${block.hasWeight ? 'grid-cols-[20px_1fr_1fr_24px]' : 'grid-cols-[20px_1fr_24px]'}`}>
                    <span className="text-xs text-gray-300 font-bold text-center">S{si+1}</span>
                    <input type="number" placeholder="—" value={set.reps}
                      onChange={e => updSet(block.id, si, { reps: e.target.value })}
                    ...
                    {block.hasWeight && (
                      <input type="number" placeholder="—" value={set.weight}
                        onChange={e => updSet(block.id, si, { weight: e.target.value })}
                        className="rounded-lg border border-gray-400 px-2 py-1.5 text-sm text-gray-900 text-center focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    )}
                    <button onClick={() => {
                      if (block.sets.length > 1)
                        setBlocks(bs => bs.map(b => b.id === block.id
                          ? { ...b, sets: b.sets.filter((_, i) => i !== si) } : b))
                    }} className="text-gray-300 hover:text-gray-500 text-base text-center">×</button>
                  </div>
                ))}
                <button onClick={() => setBlocks(bs => bs.map(b => b.id === block.id
                  ? { ...b, sets: [...b.sets, { reps: '', weight: '' }] } : b))}
            ...
            <button onClick={() => setBlocks(bs => [...bs, newBlock(Date.now())])}
              className="w-full py-3 border-2 border-dashed border-orange-200 text-orange-400 rounded-2xl text-sm font-bold hover:border-orange-400 transition">
              + Ajouter un bloc
            </button>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-gray-200 p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Conditioning work, partie métabolique</p>
              </div>
            ...
            {hasWod && <>
                  ...
                  {WOD_FORMATS.map(f => (
                    <button key={f} onClick={() => setWodFormat(f)}
                      className={`px-4 py-1.5 rounded-full text-sm font-medium border transition ${
                        wodFormat === f ? 'border-orange-400 bg-orange-50 text-orange-600' : 'border-gray-300 bg-white text-gray-600'
                      }`}>{f}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <textarea rows={4} value={wodDesc} onChange={e => setWodDesc(e.target.value)}
          ...
          <div className="space-y-6">
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                <span className="text-orange-500 text-xs">· {RPE_LABELS[rpe]}</span>
              ...
              {showRpeInfo && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-3">
                  ...
                  <p className="text-xs text-blue-600 mb-3">Note ton effort ressenti de 1 à 10, indépendamment des charges ou du chrono.</p>
                  <div className="space-y-1.5 text-xs text-blue-700">
                    {[['1–3','Très facile · conversation normale'],['4–5','Modéré · légèrement essoufflé'],['6–7','Difficile · parler devient compliqué'],['8–9','Très dur · impossible de parler'],['10','Maximum absolu · tout ce que tu as']].map(([n,l]) => (
            ...
            <div>
              ...
              {addingPain ? (
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      {SEVERITY.map(s => (
                        <button key={s.v} onClick={() => setPainSev(s.v)}
                          className={`flex-1 py-2 rounded-lg border text-xs font-bold transition ${painSev === s.v ? s.cls : 'border-gray-200 bg-white text-gray-400'}`}>
                          {s.l}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setAddingPain(false); setPainPart('') }}
                      className="flex-1 py-2 rounded-lg border border-gray-200 text-sm text-gray-500">Annuler</button>
                    <button onClick={addPain}
                      className="flex-1 py-2 rounded-lg text-white text-sm font-bold transition"
                      style={{ background: 'var(--theme-primary, #F97316)' }}>Ajouter</button>
                  </div>
                </div>
              ) : (
        ...
        <div className="flex gap-3 pt-4 pb-10">
          {step > 0 && (
            <button onClick={goBack}
              className="flex-1 py-3.5 rounded-xl border border-gray-300 text-sm font-bold text-gray-600 hover:bg-gray-100 transition">
              ← Retour
            </button>
          )}
