...
import {
  getOrgMembers, updateMembershipRole, setEmploymentStatus, removeMembership,
  type OrgMember, type Role, type EmploymentStatus,
} from '@/lib/orgs'
import { toast } from '@/lib/toast'

const ROLE_LABEL: Record<Role, string> = { owner: 'Propriétaire', coach: 'Coach', staff: 'Staff', member: 'Membre' }
...
export default function StaffPage() {
  ...
  const load = useCallback(async () => {
    ...
    setStaff(all.filter(m => m.status === 'active' && STAFF_ROLES.includes(m.role)))
    setLoading(false)
  }, [orgId])
  useEffect(() => { void load() }, [load])

  const changeRole = async (m: OrgMember, role: Role) => { await updateMembershipRole(m.membershipId, role); void load() }
  ...
  const remove = async (m: OrgMember) => {
    if (!window.confirm(`Retirer ${m.firstName ?? 'ce coach'} du staff ?`)) return
                ...
                <div key={m.membershipId} className="bg-white rounded-xl border border-gray-200 p-3">
                  ...
                  {canManage && !isOwner && (
                    <div className="flex items-center gap-2 mt-2 pl-12">
                      <select className={selCls} value={m.role} onChange={e => changeRole(m, e.target.value as Role)}>
